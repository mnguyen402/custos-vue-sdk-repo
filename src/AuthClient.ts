...
import { ref } from 'vue';
import { CUSTOS_INJECTION_KEY, CUSTOS_TOKEN } from './token';
import {bindPluginMethods} from './utils';

export interface Config {
    clientId: string;
    clientSecret: string;
    custosAuthBaseUrl: string;
    redirectUri: string;
}
...
class AuthVueClient {
    ...
    constructor(config: Config) {
        ...
        this.axiosInstance = axios.create({
            baseURL: this.config.custosAuthBaseUrl,
    ...
    install(app: App) {
        app.config.globalProperties[CUSTOS_TOKEN] = this;
        app.provide(CUSTOS_INJECTION_KEY, this as AuthVueClient);
    }
